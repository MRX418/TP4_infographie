...
public:
    ...
    void sauvegarderFenetreEnPNG( )
    {
        std::cout << "Sauvegarder en PNG ... ";
        unsigned char *pixels = new unsigned char[3*largeur_*hauteur_];
        if ( pixels )
        {
            // utiliser FreeImage : https://freeimage.sourceforge.io/
            //FreeImage_Initialise( ); // pas nécessaire si librairie dynamique

            const int bpp = 3*8;
            FIBITMAP *bitmap = FreeImage_Allocate( largeur_, hauteur_, bpp );
            if ( !bitmap )
                std::cout << "Erreur à l'allocation du Bitmap. :(" << std::endl;
            else
            {
                ...
                // remplir le bitmap avec les pixels obtenus
                for ( int i = 0 ; i < largeur_ ; i++ )
                {
                    for ( int j = 0 ; j < hauteur_ ; j++ )
                    {
                        RGBQUAD color;
                        color.rgbRed   = pixels[(3*(j*largeur_+i))+0];
                        color.rgbGreen = pixels[(3*(j*largeur_+i))+1];
                        color.rgbBlue  = pixels[(3*(j*largeur_+i))+2];
                ...
                // écrire le fichier
                char fichier[100];
                strncpy( fichier, composerNomFichier( "png" ).c_str(), sizeof(fichier) );
                if ( FreeImage_Save( FIF_PNG, bitmap, fichier, 0 ) )
